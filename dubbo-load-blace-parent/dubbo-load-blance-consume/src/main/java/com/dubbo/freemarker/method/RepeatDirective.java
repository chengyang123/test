...
public class RepeatDirective implements TemplateDirectiveModel {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		while (paramIter.hasNext()) {//变量参数
			Map.Entry ent = (Map.Entry) paramIter.next();
			String paramName = (String) ent.getKey();//获得key值
			TemplateModel paramValue = (TemplateModel) ent.getValue();
			if (paramName.equals(PARAM_NAME_COUNT)) {//如果等于count判断他的值是否为数字类型
			...
			} else if (paramName.equals(PARAM_NAME_HR)) {//如果等于hr判断他的值是否为布尔类型
				if (!(paramValue instanceof TemplateBooleanModel)) {
					throw new TemplateModelException("The \"" + PARAM_NAME_HR
							+ "\" parameter " + "must be a boolean.");
				}
				hrParam = ((TemplateBooleanModel) paramValue).getAsBoolean();//转成成布尔值
			} else {
